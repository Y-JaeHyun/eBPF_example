...
//go:generate go run github.com/cilium/ebpf/cmd/bpf2go -cc clang-15 -cflags -O2 -type key -type value bpf tcprtt.c -- -I../headers
...
func readLoop(objs bpfObjects) {
	...
	for {
		...
		*/
		iter := objs.Events.Iterate()
		if iter == nil {
			continue
		}

		if iter.Next(&key, &value) == false {
			continue
		}
